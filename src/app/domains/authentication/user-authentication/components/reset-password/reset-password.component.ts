...
export class ResetPasswordComponent implements OnInit { // Implement OnInit
  ...
  readonly ResetPasswordRequest = signal<IResetPasswordRequestDto | null>(null);
  readonly ResetPasswordResponse = signal<IResetPasswordResponseDto | null>(null);
  readonly ResetPasswordError = signal<ApiError | null>(null);
  ...
  protected onSubmit(): void {
    if (this.form.valid) {
      ...
      this.ResetPasswordRequest.set(queryParams);
    } else {
  ...
  private setupResetPasswordEffect(): void {
    effect(() => {
      const request = this.ResetPasswordRequest();
      if (!request) return;
      ...
      this.userAuthApi.resetPassword(request)
        ...
        .subscribe({
          next: (res: IResetPasswordResponseDto) => {
            this.ResetPasswordResponse.set(res);
  ...
  protected handleResetPasswordResponse(res: IResetPasswordResponseDto): void {
    ...
    if (res.status) {
      ...
      Logger.warn('Reset Password Cancelled or Failed');
    }
  }

  private openOtpMethodSelectionDialog(): void {
    Logger.info('Open OTP Reset Password dialog');
