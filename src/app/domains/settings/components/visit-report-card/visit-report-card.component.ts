...
import { ActionsDropdownMenuComponent, ModalService, LocalizationService, IActionDropdownMenuItem } from '../../../../shared';
import { ChatAppointmentMenuItemActionsConfig, SessionDetailsModalComponent, APPOINTMENT_CATEGORIES, AppointmentsRoutesEnum, ScheduleAppointmentComponent, ConfirmCancelAppointmentComponent, ICalcReservationCancelPriceData, CancelAppointmentComponent } from '../../../appointments';
...
export class VisitReportCardComponent {
  ...
  @Input({ required: false }) isReport!: boolean;
  ...
  @Input({ required: false }) hideBottomBarDetails!: boolean;
  ...
  private openCancelModal(item: IGlobalReservationModel, responseCancel: { status?: boolean, response: ICalcReservationCancelPriceData | null } | null): void {
    ...
    modalOutputs = {
      closed: () => Logger.debug('Confirm Cancel Appointment modal closed.'),
    };
